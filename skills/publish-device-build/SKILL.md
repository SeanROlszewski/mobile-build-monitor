...
```bash
make build-world DESTINATION="generic/platform=iOS" > /tmp/ios_build_<task>_device.txt 2>&1
```

- World ID app instead: `make build-id DESTINATION="generic/platform=iOS"`.
...
```bash
make build-world XCODEBUILD_BASE='xcodebuild -project "WorldApp.xcodeproj" -destination "generic/platform=iOS" -skipMacroValidation -allowProvisioningUpdates' > /tmp/ios_build_<task>_device2.txt 2>&1
```
...
- `--scheme` must be the scheme you built (`"WorldID Staging"` for World ID).
...
If the dashboard may not be running, mention: `~/.world-builds/bin/dashboard`.
